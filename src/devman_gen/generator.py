...
def _client_source(spec: BridgeSpec) -> str:
    ...
    lines.append("")
    lines.append("def set_fresh(enabled: bool = True) -> None:")
    lines.append("    # Force subsequent reads to bypass the server read cache (sticky).")
    lines.append("    _CLIENT.set_fresh(enabled)")
    lines.append("")
    lines.append("def last_meta() -> dict:")
    lines.append("    # {'cached': bool, 'ts': float} for the most recent read.")
    lines.append("    return dict(getattr(_CLIENT, 'last_meta', {}) or {})")
    lines.append("")
...
def _server_source(spec: BridgeSpec) -> str:
    ...
    lines.append("")
    # Read functions (Device_get_*) are cacheable: served from the server read
    # cache when a poll interval is set. Writes/other calls always go live.
    cacheable = sorted(n for n in function_map if n.startswith("Device_get_"))
    lines.append(f"CACHEABLE_FUNCTIONS = {pformat(cacheable, indent=2)}")
    lines.append("")
    ...
    lines.append("")
    lines.append("def _load_env_file(path: str) -> None:")
    lines.append("    # KEY=VALUE lines; '#' comments; real environment wins over the file.")
    lines.append("    with open(path, 'r', encoding='utf-8') as fh:")
    lines.append("        for raw in fh:")
    lines.append("            line = raw.strip()")
    lines.append("            if not line or line.startswith('#') or '=' not in line:")
    lines.append("                continue")
    lines.append("            key, value = line.split('=', 1)")
    lines.append("            key = key.strip()")
    lines.append("            value = value.strip().strip('\"').strip(\"'\")")
    lines.append("            if key:")
    lines.append("                os.environ.setdefault(key, value)")
    lines.append("")
    lines.append("def main() -> None:")
    lines.append("    # The env file must load before argparse defaults read os.environ.")
    lines.append("    argv = list(sys.argv[1:])")
    lines.append("    env_file = os.getenv('DEVMAN_ENV_FILE', '')")
    lines.append("    if '--env-file' in argv:")
    lines.append("        idx = argv.index('--env-file')")
    lines.append("        if idx + 1 < len(argv):")
    lines.append("            env_file = argv[idx + 1]")
    lines.append("    if env_file:")
    lines.append("        _load_env_file(env_file)")
    lines.append("    parser = argparse.ArgumentParser(description='Run devman manager server')")
    lines.append("    parser.add_argument('--env-file', default=env_file or None, help='KEY=VALUE file loaded into the environment before other defaults (DEVMAN_ENV_FILE)')")
    ...
    lines.append("    parser.add_argument('--cache-poll-interval', type=float, default=float(os.getenv('DEVMAN_CACHE_POLL_INTERVAL', '0')), help='seconds between server read-cache refreshes (0 disables; Device_get_* served from cache when enabled)')")
    ...
    lines.append("        cacheable_functions=set(CACHEABLE_FUNCTIONS),")
    lines.append("        cache_poll_interval=float(args.cache_poll_interval),")
    lines.append("    )")
    lines.append("")
